...
const coefAtualizacao = anos => Math.pow(1.02, anos);
...
const MiniSlider = ({ label, value, onChange, min, max, step, suffix = "", info }) => (
...
const Slider = ({ label, value, onChange, min, max, step, suffix = "", hint }) => (
  <div className="mb-3">
...
const KPI = ({ label, value, sub, accent, large, info }) => (
...
const PLRow = ({ label, value, type, hint }) => {
  const styles = {
  ...
  return (
    <div className={`flex justify-between py-0.5 text-sm ${styles[type] || 'text-gray-600'}`}>
...
export default function ImoSimula() {
  ...
  const analise = useMemo(() => {
    ...
    const lucroAnualizado = horizonte > 0 ? (final.lucro || 0) / horizonte : 0;
    ...
    return {
      ...
      roiFinal: final.roiPct || 0, 
      valorFinal: final.valorImovel || preco,
      lucroLiquido: final.lucro || 0,
      lucroAnualizado,
      roiAnualizado,
      equityFinal: final.equity || 0,
      equityLiquidaFinal: final.equityLiquida || 0,
      custosVendaFinal: final.custosVenda || 0,
    };
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 sticky top-[52px] z-30 shadow-sm">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <Dropdown label="Financiamento" isOpen={openDropdown === 'fin'} onToggle={v => setOpenDropdown(v ? 'fin' : null)}>
              ...
              <Dropdown label="Despesas" isOpen={openDropdown === 'desp'} onToggle={v => setOpenDropdown(v ? 'desp' : null)}>
              ...
              <Dropdown label="Projeção" isOpen={openDropdown === 'proj'} onToggle={v => setOpenDropdown(v ? 'proj' : null)}>
